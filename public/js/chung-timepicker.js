	...
	$.chungTimePicker = function(element, options) {
		...
		function createHoursWrap() {
			var table = $('<table class="table-condensed"></table>'),
				title = $('<thead><tr><th colspan="6" class="title">Часы</th></tr></thead>'),
				hours = $('<tbody></tbody>'),
		...
		function createMinutesWrap() {
			...
			switch(obj.options.viewType) {
				case 0:
					title.find('tr').append('<th class="prev js-back-hours"><i class="icon-arrow-left"></i></th><th colspan="4" class="title">Минуты</th>');
					break;
				case 1:
					title.find('tr').append('<th colspan="6" class="title">Минуты</th>');
					break;
				default:
					console.log(obj.errorPrefix + 'viewType Error');
					break;
