    ...
    <section class="lk-event">
        <div class="center-wrap">
            ...
            <div class="lk-event__nav">
                <a href="/lk-event/{{$event->id}}/proposal/" class="lk-event__nav-item lk-event__nav-item--active btn">Заявки</a>
                <a href="/lk-event/{{$event->id}}/users/" class="lk-event__nav-item btn">Участники</a>
            </div>
            <div class="lk-event__list">
                @foreach($proposals as $item => $key)
                    ...
                    <div class="lk-event__item-right">
                        <div class="lk-event__item-text">
                            Придется пользоваться их услугами, т.к. единственное что мне важно - объем жесткого диска.. надеюсь у вас появится такая возможность в будущем.
                        </div>
                        <div class="lk-event__item-btns">
                            <a href="/lk-event/{{$key->event_id}}/accept/{{$key->id}}" class="btn">Принять</a>
                            <a href="/lk-chat.html">перейти в чат</a>
                            <a href="/lk-event/{{$key->event_id}}/delete/{{$key->id}}">отклонить</a>
                        </div>
                    </div>
                </div>
                @endforeach
